...
if [ $# != 2 ];then
  echo 'deploy argument [snapshot(s for short) | release(r for short) | DEPLOY_PATH] needed!'
  exit 0
fi
...
deployFunc(){
  ...
  cd $CURRENT_PATH
  # 开始deploy
  mvn clean deploy -Dmaven.test.skip  -DaltDeploymentRepository=self-mvn-repo::default::file:$DEPLOY_PATH/repository
